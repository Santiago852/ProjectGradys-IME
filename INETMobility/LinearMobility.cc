...
void LinearMobility::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        rad latitude = deg(fmod(par("initialLatitude").doubleValue(), 360));
        rad longitude = deg(fmod(par("initialLongitude").doubleValue(), 360));
        m altitude = m(par("initialAltitude"));
        //vamos analisar se temos de mudar essa ultima linha
